...
function getRevealedCharacteristics(game) {
  ...
  game.players.forEach(player => {
    ...
    if (player.status === 'kicked' || player.status === 'dead') {
      return; // переходим к следующему игроку
    }
...
2. Событие всегда вызвано ВНЕШНИМ фактором (животные, катастрофа, погодное явление, заброшенные объекты, мародеры, техногенные аварии и т.д.).
   Запрещены внутренние конфликты между игроками.
3.  Событие может быть:
   - только негативным
   - только позитивным
   - смешанным (и негативным, и позитивным)

   НО:
   - последствия должны затрагивать конкретных игроков или ресурсы
   - нельзя делать абстрактное общее улучшение или общее ухудшение
   - даже если событие полностью негативное, в нем должна быть логика и конкретика

4.  Последствия должны быть ЛОГИЧНЫМИ и причинно-следственными.
   Нельзя:
   - абстрактные бонусы (харизма, убеждение и т.д.)
   - нелогичное лечение (например антибиотик не лечит слепоту)
   - "здоровье всех улучшилось"
5.  Изменения ресурсов должны быть РЕАЛИСТИЧНЫМИ и КОНКРЕТНЫМИ:
   - Еда увеличивается максимум на +2 года
   - Еда уменьшается максимум на -1 год
   - Нельзя писать "до 2 лет" или диапазоны
   - Количество должно соответствовать найденным ресурсам
6.  Можно:
   - ухудшать или улучшать здоровье конкретного игрока
   - добавлять фобии
   - добавлять или забирать предметы инвентаря
   - менять дополнительные характеристики бункера, например ухудшение вентиляции от внешнего фактора или что-то позитивное.
7.  Событие должно быть необычным, жестким, допускается черный юмор.
   С вероятностью 70% допускается пошлый или неловко-интимный элемент (но без откровенной порнографии).
8.  Никакой вариативности формулировок. Все изменения должны быть четкими и конкретными.
9. Новое событие не должно быть похожим на предыдущие 2 события.
10. Событие не должно создавать требований для конкретного игрока, например "требуется антибиотик в бункере".
11. Событие должно содержать неловкий или позорный момент одного из игроков.
12. Игроки не могут терять что-либо, например еду, если у них в инвентаре ее не было. Все должно быть конкретно. Максимум что можно потерять или повредить это части одежды.
13. Здоровье может меняться только в рамках: 1. ухудшения или улучшения текущей степени болезни, 2. полного излечения (на "здоров"), 3. либо добавление новой болезни и степени (легкая,средняя,тяжелая, критическая), не должно быть неопределенных подобных ответов: "Здоровье: -10% (стресс)", стресс это вообще не болезнь.
14. Не должно быть событий, блокирующих полностью возможность попадения в текущий бункер, например кража игроков кем-либо, потеря где-то в лесу, переход в другой бункер,

=== ФОРМАТ ОТВЕТА ===

3-4 предложений.

Затем отдельным блоком:

<br> Последствия:
<br> - Имя игрока: конкретное изменение здоровья
<br> - Имя игрока: новая фобия (если есть)
<br> - Инвентарь: ...
<br> - Запас еды: +X месяцев / -X месяцев / +X лет / -X лет
