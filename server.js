...
function generateEventPrompt(game) {
  ...
  // 1️⃣ Собираем только активных игроков
  const activePlayers = [];
  game.players.forEach(player => {
  ...
  // 2️⃣ Берём раскрытые характеристики только активных
  const revealedAll = getRevealedCharacteristics(game);
  const revealed = {};
  Object.entries(revealedAll || {}).forEach(([playerName, chars]) => {
    if (activeNames.has(playerName)) revealed[playerName] = chars;
  });

  // 3️⃣ Последние события
  const recentEvents = game.events?.slice(0, 8) || [];
  const lastEvent = recentEvents[0];

  const lastType =
    (lastEvent?.type && String(lastEvent.type).toLowerCase()) ||
    (String(lastEvent?.text || "").toLowerCase().includes("тип события: негатив") ? "negative" :
     String(lastEvent?.text || "").toLowerCase().includes("тип события: позитив") ? "positive" :
     String(lastEvent?.text || "").toLowerCase().includes("тип события: смеш") ? "mixed" : "");

  // 4️⃣ Правило: не 2 негативных подряд
  const typeRule = (lastType === "negative")
    ? `🚫 Предыдущее событие было НЕГАТИВНЫМ. Текущее НЕ МОЖЕТ быть НЕГАТИВНЫМ.
Сгенерируй либо ПОЗИТИВНОЕ, либо СМЕШАННОЕ с перевесом плюсов.`
    : `Можно любой тип, но не повторяй механику последних событий.`;

  // 5️⃣ Профили активных игроков
  const profilesText = Object.entries(revealed).length
    ? Object.entries(revealed).map(([name, chars]) => {
        const parts = [];
        if (chars.gender) parts.push(`пол/возраст=${chars.gender}`);
        if (chars.bodyType) parts.push(`телосложение=${chars.bodyType}`);
        if (chars.trait) parts.push(`черта=${chars.trait}`);
        if (chars.profession) parts.push(`профессия=${chars.profession}`);
        if (chars.hobby) parts.push(`хобби=${chars.hobby}`);
        if (chars.health) parts.push(`здоровье=${chars.health}`);
        if (chars.phobia) parts.push(`фобия=${chars.phobia}`);
        if (chars.inventory) parts.push(`инвентарь=${chars.inventory}`);
        if (chars.extra) parts.push(`доп=${chars.extra}`);
        return `- ${name}: ${parts.join("; ") || "нет раскрытых данных"}`;
      }).join("\n")
    : "- Нет раскрытых характеристик.";

  // 6️⃣ Инвентарь активных
  const inventoryText = Object.entries(revealed)
    .map(([name, chars]) =>
      `  - ${name}: ${chars.inventory ? chars.inventory : "инвентарь не раскрыт"}`
    )
    .join("\n");

  const recentEventsText = recentEvents.length
    ? recentEvents.map(e => `  - ${String(e.text || "").slice(0, 150)}...`).join("\n")
    : "  - Событий пока не было";

  // ✅ 7️⃣ БУНКЕР: добавляем структуру и разбор особенностей
  const bunker = game.bunker || {};
  const bunkerExtra = String(bunker.extra || "").toLowerCase();

  // Простой парсер "фич" из текста extra (не обязателен, но помогает модели)
  const bunkerFeatures = [];
  const addFeature = (cond, text) => { if (cond) bunkerFeatures.push(`- ${text}`); };

  addFeature(bunkerExtra.includes("бетон"), "Толстые бетонные стены (высокая защита)");
  addFeature(bunkerExtra.includes("герметич"), "Герметичные двери (хорошая изоляция)");
  addFeature(bunkerExtra.includes("жизнеобеспеч"), "Автономная система жизнеобеспечения");
  addFeature(bunkerExtra.includes("дизел"), "Дизель-генератор");
  addFeature(bunkerExtra.includes("топлив"), "Есть запас топлива");
  addFeature(bunkerExtra.includes("фильтр") || bunkerExtra.includes("фильтрац"), "Система фильтрации воздуха");
  addFeature(bunkerExtra.includes("скважин") || bunkerExtra.includes("артезиан"), "Есть артезианская скважина (источник воды)");
  addFeature(bunkerExtra.includes("кровать"), "Есть спальные места");
  addFeature(bunkerExtra.includes("слом"), "Есть неисправность/поломка внутри бункера (например, сломанная кровать)");

  const bunkerStructureText = `
🏚️ БУНКЕР (структура и ресурсы):
- Срок проживания: ${bunker.duration_years ?? "?"} лет
- Запас еды: ${bunker.food_years ?? "?"} лет
- Описание: ${bunker.extra || "нет описания"}

Ключевые особенности бункера (используй в событии, если логично):
${bunkerFeatures.length ? bunkerFeatures.join("\n") : "- нет выделенных особенностей"}
`.trim();
...
⚠️ Предыдущие события (не повторяй механику):
${recentEventsText}
...
👥 Активные игроки (изгнанные и мертвые НЕ учитываются):
${profilesText}

📦 Инвентарь активных игроков:
${inventoryText || "  - Нет данных"}

Разрешено изменять ТОЛЬКО:
- inventory — только из GAME_DATA.characteristics.inventory
- phobia — только из GAME_DATA.characteristics.phobias (БЕЗ степеней!)
- extra — только из GAME_DATA.characteristics.extras
- health — только из GAME_DATA.characteristics.health + степень из HEALTH_SEVERITIES
- profession — только из GAME_DATA.characteristics.professions[].name
- trait/bodyType/hobby — только из соответствующих массивов

🚫 Запрещено:
- менять нераскрытые характеристики
- придумывать новые значения, которых нет в массивах
- менять степень фобии (у фобий степеней НЕТ)

Игроки находятся СНАРУЖИ бункера у входа. 
Игроки не спят.
Нельзя убивать игроков.
Событие должно иметь логическое завершение.
Все последствия событий должны иметь логическую связь и быть максимально конкретными.
Найденный предмет добавляется в инвентарь к конкретному игроку.
Внешние новые персонажи, появляющиеся в событии, не должны отражаться в последствиях (например, если они ранены и так далее), а также в конечном результате они не могут попасть в бункер, но при этом могут безуспешно пытаться (уговоры, угрозы, насилие и так далее).
В последствиях строго информация о бункере, запасе еды, игроках.

Тип события строго один:
🔴 НЕГАТИВНОЕ — только минусы
🟢 ПОЗИТИВНОЕ — только плюсы
🟡 СМЕШАННОЕ — плюсы и минусы

=== ФОРМАТ ===
8–10 предложений. Каждое последствие с нового абзаца (добавляй "<br>")

<br> Тип события: [НЕГАТИВНОЕ / ПОЗИТИВНОЕ / СМЕШАННОЕ]

<br> Последствия:
<br> - Игрок: здоровье: "X (средняя) → X (тяжелая)" или "Здоров → Y (легкая)"
<br> - Игрок: фобия: "Добавлена фобия: ..."
<br> - Игрок: инвентарь: "Потерян предмет: ..." / "Добавлен предмет: ..."
<br> - Запас еды: +X месяцев / -X месяцев
<br> - Бункер: конкретное изменение/повреждение/улучшение (если логично)
`.trim();
