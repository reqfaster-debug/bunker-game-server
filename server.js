...
function generateEventPrompt(game) {
  ...
  if (recentEvents.length > 0) {
    ...
    recentEvents.forEach(event => {
      ...
      // Добавляем конкретные слова из события
  ...
  const explicitCategories = [
    ...
    "Инцест и извращения: родственные связи, зоофилия (только упоминание), некрофилия",
    "Проституция: торговля телом, секс за еду, секс за защиту, секс за лекарства, секс за место в бункере",
    "Порнография: найденные журналы, видео, фото, игрушки, резиновые женщины",
    "Онанизм: мастурбация, самоудовлетворение, порнозависимость",
    "Гомосексуальные сцены: мужские и женские контакты, лесбиянство, гей-пары",
    "Групповой секс: оргии, свингеры, обмен партнерами",
    "Фетишизм: БДСМ, кожа, латекс, резина, связывание, ролевые игры",
    "Неловкие ситуации: случайно увидел голым, застрял в узком месте",
    "Телесные жидкости: сперма, моча, кал, кровь, рвота, гной",
    "Гигиена: вши, блохи, грязь, запах, немытость",
    "Одежда: порванная, грязная, чужая, женская/мужская одежда, лифчики, трусы",
    "Случайные прикосновения: схватил за грудь, за задницу, за яйца",
    "Эрекция и поллюции: стояк в неподходящий момент, мокрые сны",
    "Менструация: месячные, прокладки, тампоны, боли, кровь на одежде",
    "Беременность: зачатие, роды, выкидыш, аборт",
    "Половые болезни: сифилис, гонорея, триппер, СПИД, герпес",
    "Импотенция и фригидность: не стоит, не хочу, не могу"
  ];
  
  // Добавляем пошлые категории с вероятностью 50%
  if (!bannedThemes.has('пошлость') && Math.random() < 0.5) {
  ...
  // Получаем последние 5 событий для контекста
...
=== ВАЖНОЕ УСЛОВИЕ ===
Группа НИКОГДА НЕ СПИТ. Они постоянно находятся у входа в бункер, спорят, ждут, бодрствуют.
События НЕ МОГУТ происходить, когда группа спит — они всегда активны и начеку.
...
3. ТИП СОБЫТИЯ (выбери ОДИН):
   
   🔴 НЕГАТИВНОЕ (40% вероятность):
   - Только негативные последствия (потеря еды, ухудшение здоровья, потеря инвентаря)
   - Никаких находок или плюсов
   - Пример: нападение, кража, травма, болезнь, домогательство
   
   🟢 ПОЗИТИВНОЕ (20% вероятность):
   - Только позитивные последствия (находка еды, улучшение здоровья, новый инвентарь)
   - Никаких потерь или ухудшений
   - Пример: найден тайник с едой, код от сейфа в бункере, полезный инструмент
   - ПОЗИТИВНОЕ событие может давать: еду (+месяцы), инвентарь, улучшение характеристик бункера, лечение болезней
   
   🟡 СМЕШАННОЕ (40% вероятность):
   - Есть и позитивные, и негативные последствия
   - Должно быть логическое обоснование обоих типов
   - Пример: отбились от мародеров, но потеряли рюкзак, зато забрали у них нож

4. ВАЖНО: Не путай типы!
   - В НЕГАТИВНОМ событии НЕ МОЖЕТ быть находок или улучшений
   - В ПОЗИТИВНОМ событии НЕ МОЖЕТ быть потерь или ухудшений
   - СМЕШАННОЕ событие должно содержать и то, и другое

5. ФОРМУЛИРОВКА ЕДЫ:
   - Не нужно перечислять конкретные продукты (тушенка, консервы, крупа)
   - Используй обобщенные формулировки:
     ✅ "Нашли запас еды на 6 месяцев"
     ✅ "Потеряли продуктов на 3 месяца"
     ✅ "Обнаружили тайник с едой на год"
     ❌ "Нашли 10 банок тушенки"
     ❌ "Обнаружили мешок крупы"

6. ОБЯЗАТЕЛЬНО должен быть (вне зависимости от типа):
   - Сексуальный или пошлый элемент (30% вероятность)
   - Неловкий/позорный момент для конкретного игрока (30% вероятность)
   - Элемент насилия или жестокости (40% вероятность)

7. ЗАПРЕЩЕНО заставлять игроков терять предметы, которых у них НЕТ в инвентаре!
   Доступные предметы у игроков:
${availableItems || '   - Нет раскрытого инвентаря'}

8. Если предмет не указан в инвентаре, его нельзя потерять, порвать или испортить.
   Игроки могут находить НОВЫЕ предметы, но не терять то, чего у них нет.

9. КРИТИЧЕСКИ ВАЖНО: Все последствия должны быть ЛОГИЧНЫМИ и ВЫТЕКАТЬ из события!
   ✅ ПРАВИЛЬНО: "Мародеры украли рюкзак с припасами" → Запас еды: -3 месяца
   ✅ ПРАВИЛЬНО: "Камень попал в голову" → Здоровье: Мигрень (тяжелая) → Мигрень (критическая)
   ✅ ПРАВИЛЬНО: "Нашли тайник с едой" → Запас еды: +6 месяцев, Инвентарь: добавлен консервный нож
...
10. ЗАПРЕЩЕНЫ любые абстрактные изменения:
...
11. Последствия должны быть КОНКРЕТНЫМИ и добавляться явно:
    - Здоровье: конкретное изменение болезни (например "Астма (легкая) → Астма (средняя)")
    - Фобия: "Добавлена фобия: [название]"
    - Инвентарь: "Добавлен предмет: [название]" или "Потерян предмет: [название]"
    - Еда: +X месяцев / -X месяцев (макс +2 года, -1 год)
    - Бункер: конкретное изменение характеристики (например "Найден код от сейфа в бункере с едой на 6 месяцев")

12. Здоровье может меняться только в рамках:
    - Ухудшение/улучшение текущей степени болезни (легкая/средняя/тяжелая/критическая)
    - Полное излечение (на "Здоров")
    - Добавление новой болезни со степенью

13. ЛОГИЧЕСКОЕ ЗАВЕРШЕНИЕ СОБЫТИЯ:
    Любое событие с угрозой (нападение, катастрофа, опасность) должно иметь РАЗРЕШЕНИЕ:
    - Группа должна суметь отбиться, справиться, убежать или пережить угрозу
    - Но при этом понести какие-то потери или получить что-то взамен
    - Нельзя оставлять ситуацию в подвешенном состоянии "и тут всё закончилось"
    
    ✅ ПРАВИЛЬНО: "Мародеры напали, но группа отбилась камнями. В суматохе потеряли рюкзак с припасами, зато забрали у бандитов нож"
    ✅ ПРАВИЛЬНО: "Лавина накрыла лагерь, но все успели спрятаться за скалу. Однако Афанасий вывихнул ногу, спасая Клавдию"
    ✅ ПРАВИЛЬНО: "Стая волков окружила группу, но отпугнула их факелами. В суматохе сгорела последняя палатка"
    ✅ ПРАВИЛЬНО: "Нашли заброшенный схрон с едой на год" (позитивное — нет угрозы, только находка)
    
    ❌ НЕПРАВИЛЬНО: "Напали мародеры и всё..." (нет завершения)
    ❌ НЕПРАВИЛЬНО: "Лавина сошла и накрыла лагерь" (нет последствий)
    ❌ НЕПРАВИЛЬНО: "Волки напали, но все выжили" (нет конкретики)

=== ПРИМЕР УДАЧНОГО СОБЫТИЯ (НЕГАТИВНОЕ С ПОШЛЫМ МОМЕНТОМ) ===
"Пока группа спорит у входа в бункер, из-за кустов за ними наблюдает местный сумасшедший по кличке Глаз. Когда Анна отходит в сторону, чтобы справить нужду, Глаз выскакивает из кустов, зажимая ей рот рукой и прижимаясь сзади, шепча пошлости о том, как он 'мечтал потрогать такую красивую попку'. Анна отбивается локтем и кусает его за руку, после чего Глаз с визгом убегает в лес, роняя старый ржавый нож. На крики прибегают остальные, но в спешке спотыкаются о коряги — Дмитрий падает и ломает очки, без которых почти ничего не видит. Анна теперь постоянно оглядывается и вздрагивает от каждого шороха."

<br> Тип события: НЕГАТИВНОЕ

<br> Последствия:
<br> - Анна: Добавлена фобия: Скоптофобия (боязнь слежки)
<br> - Дмитрий: Добавлена близорукость (легкая)
<br> - Инвентарь: добавлен ржавый нож
<br> - Запас еды: без изменений
...
`;
  ...
  prompt += `\n=== ФОРМАТ ОТВЕТА ===
8-10 предложений с детальным описанием события (в два раза больше обычного).

ВАЖНО: Каждое последствие должно логически вытекать из описанного события. Не добавляй ничего, что не описано в тексте!

<br> Тип события: [НЕГАТИВНОЕ / ПОЗИТИВНОЕ / СМЕШАННОЕ]

<br> Последствия:
<br> - Имя игрока: конкретное изменение здоровья (если есть)
<br> - Имя игрока: добавлена фобия: [название] (если есть)
<br> - Инвентарь: добавлен предмет: [название] / потерян предмет: [название]
<br> - Запас еды: +X месяцев / -X месяцев (обобщенно, без конкретных продуктов)
<br> - Бункер: изменение (если есть, например "Найден код от сейфа с едой на 6 месяцев")

🔴 КРИТИЧЕСКИ ВАЖНО:
1. ГРУППА НИКОГДА НЕ СПИТ — все события происходят, когда они бодрствуют у входа в бункер!
2. НИКАКИХ абстрактных понятий (мораль, настроение, отношения)!
3. Каждое последствие должно быть ОБОСНОВАНО в тексте события!
4. ПОЗИТИВНОЕ событие — ТОЛЬКО плюсы!
5. НЕГАТИВНОЕ событие — ТОЛЬКО минусы!
6. СМЕШАННОЕ событие — и плюсы, и минусы!
7. ЕДА — только обобщенно (+X месяцев / -X месяцев), без конкретных продуктов!
8. Опиши событие ПОДРОБНО — 8-10 предложений!
9. УБЕДИСЬ, что у события есть логическое завершение (пункт 13 правил)!`;
