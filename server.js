...
function generateEventPrompt(game) {
  ...
  // Собираем раскрытые характеристики ТОЛЬКО активных игроков
  const revealedChars = {};
  activePlayers.forEach(player => {
    const chars = player.characteristics || {};
    const out = {};
    Object.entries(chars).forEach(([key, obj]) => {
      if (obj && obj.revealed) out[key] = obj.value;
    });
    if (Object.keys(out).length > 0) revealedChars[player.name] = out;
  });

  // Вытаскиваем раскрытый инвентарь активных игроков (для запрета «потерь того, чего нет»)
  const playerInventories = {};
  Object.entries(revealedChars).forEach(([name, chars]) => {
    if (chars.inventory) playerInventories[name] = chars.inventory;
  });

  const availableItemsByPlayersText = Object.keys(playerInventories).length
    ? Object.entries(playerInventories).map(([name, item]) => `  - ${name}: ${item}`).join('\n')
    : '  - Нет раскрытого инвентаря';

  // Все разрешённые значения (строго из массивов)
  const ALLOWED = {
    inventory: GAME_DATA.characteristics.inventory || [],
    phobias: GAME_DATA.characteristics.phobias || [],
    extras: GAME_DATA.characteristics.extras || [],
    traits: GAME_DATA.characteristics.traits || [],
    bodyTypes: GAME_DATA.characteristics.bodyTypes || [],
    hobbies: GAME_DATA.characteristics.hobbies || [],
    professions: (GAME_DATA.characteristics.professions || []).map(p => p?.name).filter(Boolean),
    health: (GAME_DATA.characteristics.health || []).map(h => h?.name).filter(Boolean)
  };

  // Последние события
  const recentEvents = (game.events || []).slice(0, 7);

  // Пытаемся определить тип последнего события (лучше, если ты сохраняешь event.type отдельно)
  function getLastEventType() {
    const last = recentEvents[0];
    if (!last) return null;
    if (last.type) return String(last.type).toUpperCase();

    const t = String(last.text || '').toUpperCase();
    if (t.includes('ТИП СОБЫТИЯ:')) {
      if (t.includes('НЕГАТИВ')) return 'НЕГАТИВНОЕ';
      if (t.includes('ПОЗИТИВ')) return 'ПОЗИТИВНОЕ';
      if (t.includes('СМЕШАН')) return 'СМЕШАННОЕ';
    }
    return null;
  }

  const lastType = getLastEventType();

  // Правило «не 2 негативных подряд»
  // Если прошлое было негативным — следующее НЕ может быть негативным.
  // И должно быть «больше позитива, чем негатива» => принудительно Mixed(P+) или Positive.
  let forcedTypeRule = '';
  if (lastType === 'НЕГАТИВНОЕ') {
    forcedTypeRule = `
ОСОБОЕ ПРАВИЛО ПО СЕРИИ:
- Предыдущее событие было НЕГАТИВНЫМ.
- СЛЕДУЮЩЕЕ событие НЕ МОЖЕТ быть НЕГАТИВНЫМ.
- Выбирай ТОЛЬКО: ПОЗИТИВНОЕ или СМЕШАННОЕ, но в СМЕШАННОМ должно быть БОЛЬШЕ ПОЗИТИВА, чем негатива (например 2 плюса и 1 минус).`;
  }

  // Запрещаем повтор тем/слов из недавних событий (как у тебя, только компактнее)
  const bannedThemes = new Set();
  const bannedKeywords = new Set();

  const keywordMap = {
    'собак': 'животные', 'пёс': 'животные', 'волк': 'животные', 'медвед': 'животные',
    'секс': 'пошлость', 'трах': 'пошлость', 'раздет': 'пошлость',
    'труп': 'смерть', 'мертв': 'смерть', 'убит': 'смерть',
    'кров': 'кровь', 'ранен': 'травмы', 'перелом': 'травмы', 'ожог': 'травмы',
    'лес': 'природа', 'болот': 'природа',
    'рек': 'вода', 'озер': 'вода', 'вод': 'вода',
    'дожд': 'погода', 'снег': 'погода', 'ветер': 'погода',
    'мороз': 'холод',
    'пожар': 'огонь', 'взрыв': 'взрыв',
    'оружи': 'оружие', 'нож': 'оружие', 'топор': 'оружие',
    'аптечк': 'медицина', 'лекарств': 'медицина', 'антибиот': 'медицина',
    'консерв': 'еда', 'тушенк': 'еда', 'сухпай': 'еда'
  };

  recentEvents.forEach(ev => {
    const text = String(ev.text || '').toLowerCase();
    Object.entries(keywordMap).forEach(([kw, theme]) => {
      if (text.includes(kw)) {
        bannedThemes.add(theme);
        bannedKeywords.add(kw);
      }
    });
    text.split(/\s+/).slice(0, 14).forEach(w => {
      const ww = w.replace(/[^\p{L}\p{N}-]+/gu, '');
      if (ww.length > 4) bannedKeywords.add(ww);
    });
  });

  const bannedThemesList = bannedThemes.size
    ? Array.from(bannedThemes).map(t => `  - ${t}`).join('\n')
    : '  - нет';

  const bannedKeywordsList = bannedKeywords.size
    ? Array.from(bannedKeywords).slice(0, 14).map(w => `  - "${w}"`).join('\n')
    : '  - нет';

  const recentEventsText = recentEvents.length
    ? recentEvents.map(e => `  - "${String(e.text || '').slice(0, 140)}..."`).join('\n')
    : '  - Событий пока не было';

  // Описание катастрофы/бункера
  const disasterText = game.disaster || 'неизвестно';
  const bunker = game.bunker || {};
  const bunkerText = `Срок: ${bunker.duration_years ?? '?'} лет; еды: ${bunker.food_years ?? '?'} лет; описание: ${bunker.extra ?? 'нет описания'}`;

  // Категории — можешь расширять, но с учётом bannedThemes
  const disasterType = String(disasterText).toLowerCase();
  const categories = [];

  // Ядерка
  if ((disasterType.includes('ядерн') || disasterType.includes('радио')) && !bannedThemes.has('радиация')) {
    categories.push(
      'Радиация и заражение: пятна, пыль, заражённая вода, дозиметры, фильтры',
      'Лучевая болезнь и последствия облучения (без убийств)',
      'Разрушенные объекты и радиоактивный мусор'
    );
  }

  // Пандемия
  if ((disasterType.includes('пандем') || disasterType.includes('вирус') || disasterType.includes('болезн')) && !bannedThemes.has('болезни')) {
    categories.push(
      'Карантин и заражённые зоны, проверки, дезинфекция',
      'Медицинские находки (строго из разрешённого инвентаря)'
    );
  }

  // Зомби
  if ((disasterType.includes('зомб') || disasterType.includes('ходяч')) && !bannedThemes.has('зомби')) {
    categories.push(
      'Заражённые/зомби-подобные угрозы без убийств игроков',
      'Баррикады, отвлечение угрозы, спасение ценного'
    );
  }

  // Климат/вода
  if ((disasterType.includes('климат') || disasterType.includes('затоп') || disasterType.includes('вода')) && !bannedThemes.has('вода')) {
    categories.push(
      'Ледяная вода, размывы, провалы грунта, затопленные объекты',
      'Поиск безопасного пути и ресурсов'
    );
  }

  // Вечная зима
  if ((disasterType.includes('зим') || disasterType.includes('мороз') || disasterType.includes('холод')) && !bannedThemes.has('холод')) {
    categories.push(
      'Обморожения/гипотермия (без убийств), борьба с холодом',
      'Снег, лёд, лавины, трещины, скользкие поверхности'
    );
  }

  // База всегда
  categories.push(
    'Мародёры/конкуренты/беженцы (без убийств игроков)',
    'Техногенная опасность рядом с входом (обрыв, пожар, утечка, обвал)',
    'Заброшенный объект поблизости (склад, будка, тоннель, люк, КПП)',
    'Насекомые/плесень/споры/грязь и санитарные проблемы'
  );

  // Фильтруем по bannedThemes слегка (если тема совпадает)
  const filteredCategories = categories.filter(c => {
    const t = c.toLowerCase();
    if (bannedThemes.has('вода') && t.includes('вод')) return false;
    if (bannedThemes.has('огонь') && t.includes('пож')) return false;
    if (bannedThemes.has('оружие') && t.includes('оруж')) return false;
    if (bannedThemes.has('медицина') && t.includes('мед')) return false;
    return true;
  });

  const shuffled = [...new Set(filteredCategories)].sort(() => 0.5 - Math.random());
  const selectedCategories = shuffled.slice(0, 6).map(c => `  - ${c}`).join('\n');

  // Игроки + раскрытые характеристики (чтобы LLM реально использовал возраст/телосложение/черты/проф/хобби/доп)
  const playersContext = activePlayers.map(p => {
    const chars = p.characteristics || {};
    // Даже скрытые — просим учитывать как «поведение», но менять только раскрытые
    const getVal = (k) => chars?.[k]?.value ?? 'неизвестно';
    return `- ${p.name}:
  • gender+age: ${getVal('gender')}
  ...
  • phobia: ${getVal('phobia')}
  • extra: ${getVal('extra')}`;
  }).join('\n');

  // Список разрешённых значений (укороченный в промпте, но строгий)
  // (Слишком длинные массивы лучше не вставлять целиком — но ИМЕННО инвентарь/фобии тебе критичны,
  // поэтому даём запрет и правило «строка должна совпасть полностью», плюс вставляем реальные списки.)
  const allowedInventoryText = ALLOWED.inventory.map(i => `  - ${i}`).join('\n');
  const allowedPhobiasText = ALLOWED.phobias.map(p => `  - ${p}`).join('\n');

  // ===== PROMPT =====
  let prompt = `
Уникальный seed: ${SEED}

⚠️ ПРЕДЫДУЩИЕ СОБЫТИЯ (НЕ ПОВТОРЯЙ темы/ключевые слова и последствия):
${recentEventsText}
...
🎲 РАЗРЕШЁННЫЕ КАТЕГОРИИ ДЛЯ НОВОГО СОБЫТИЯ (выбери одну):
${selectedCategories}

${forcedTypeRule}

=== КОНТЕКСТ МИРА ===
Катастрофа: ${disasterText}

Бункер:
- ${bunkerText}

Игроки (учитывай ВСЕ характеристики при описании поведения/реакций, чтобы событие было уникальным под состав группы):
${playersContext}

=== ГЛАВНОЕ ПРАВИЛО УНИКАЛЬНОСТИ ===
- Ты обязан сделать событие НЕПОХОЖИМ на последние 5–7 событий: другая ситуация, другой механизм угрозы/выгоды, другие последствия.
- Запрещено повторять уже бывшие последствия (типа «снова потеряли еду на 3 месяца» тем же способом, снова та же травма, снова те же мародёры и т.п.).

=== ЖЁСТКИЕ ПРАВИЛА (нарушение = ответ считается неправильным) ===
1) Событие НЕ МОЖЕТ убивать игроков (никаких смертей игроков).
2) Группа НЕ СПИТ. Событие происходит только когда они бодрствуют у входа в бункер.
3) Событие вызвано ВНЕШНИМ фактором (из выбранной категории выше).
4) Никаких абстрактных последствий: нельзя «мораль», «настроение», «отношения».
5) Все последствия должны быть логичными и прямо вытекать из текста события.

=== ПРАВИЛА ИЗМЕНЕНИЯ ХАРАКТЕРИСТИК (ОЧЕНЬ ВАЖНО) ===
- Менять/добавлять можно ТОЛЬКО характеристики АКТИВНЫХ игроков (НЕ kicked/НЕ dead).
- Менять можно ТОЛЬКО те характеристики, которые УЖЕ существуют как поля: gender, bodyType, trait, profession, hobby, health, inventory, phobia, extra.
- НЕЛЬЗЯ придумывать новые типы характеристик.
- НЕЛЬЗЯ менять «степень фобии» — у фобии НЕТ степеней. Фобия может только ДОБАВЛЯТЬСЯ или УБИРАТЬСЯ (если ты вообще даёшь такое последствие).
- Здоровье: допускается только формат "Болезнь (легкая/средняя/тяжелая/критическая)" либо "Здоров".
- Любое добавление/изменение значения ДОЛЖНО быть строго из соответствующего массива игры:
  • inventory — ТОЛЬКО из списка "ALLOWED INVENTORY" ниже (строка должна совпасть полностью!)
  • phobia — ТОЛЬКО из списка "ALLOWED PHOBIAS" ниже (строка должна совпасть полностью!)
  • trait — ТОЛЬКО из GAME_DATA.characteristics.traits
  • bodyType — ТОЛЬКО из GAME_DATA.characteristics.bodyTypes
  • hobby — ТОЛЬКО из GAME_DATA.characteristics.hobbies
  • profession — ТОЛЬКО из GAME_DATA.characteristics.professions (имя профессии из списка)
  • extra — ТОЛЬКО из GAME_DATA.characteristics.extras
- Запрещено добавлять в инвентарь НЕСУЩЕСТВУЮЩИЕ предметы: никаких "щенок", "метеоритные осколки", "артефакты" и т.п.

=== ПРО ИНВЕНТАРЬ (потери/находки) ===
- Запрещено заставлять игрока терять предмет, если он НЕ указан в ЕГО раскрытом инвентаре.
Доступные раскрытые предметы игроков:
${availableItemsByPlayersText}

- Добавлять новый предмет можно ТОЛЬКО из списка разрешённого инвентаря ниже.

ALLOWED INVENTORY (можно добавлять ТОЛЬКО эти строки, без изменений):
${allowedInventoryText}

ALLOWED PHOBIAS (можно добавлять ТОЛЬКО эти строки, без изменений):
${allowedPhobiasText}

=== ТИП СОБЫТИЯ (выбери один) ===
🔴 НЕГАТИВНОЕ:
- Только минусы (потери еды/предметов, ухудшение болезни, новая болезнь, добавление фобии, позор)
- Никаких находок/улучшений

🟢 ПОЗИТИВНОЕ:
- Только плюсы (еда +месяцы, найденный предмет из ALLOWED INVENTORY, излечение/улучшение болезни, улучшение параметра бункера)
- Никаких потерь/ухудшений

🟡 СМЕШАННОЕ:
- И плюсы, и минусы
- Оба типа должны быть логичны

=== ЕДА ===
- Не перечисляй продукты. Пиши только так:
  "Запас еды: +X месяцев" или "Запас еды: -X месяцев"
- Ограничения: максимум +24 месяца, минимум -12 месяцев

=== ОПИСАНИЕ ===
- 8–10 предложений, подробно, с логическим завершением (если угроза — группа должна суметь отбиться/уйти/пережить).
- Обязательно учитывай возраст/телосложение/черты/профессию/хобби/доп. сведения игроков в том, как они действуют и почему последствия именно такие.

=== ФОРМАТ ОТВЕТА (строго) ===
(1) 8–10 предложений описания.

(2) Пустая строка.
Тип события: НЕГАТИВНОЕ / ПОЗИТИВНОЕ / СМЕШАННОЕ

(3) Пустая строка.
Последствия:
- Каждое последствие ОБЯЗАТЕЛЬНО с новой строки И С ПУСТОЙ СТРОКОЙ МЕЖДУ ПУНКТАМИ.
- Формат пункта: "Имя: ..."

Пример оформления последствий (с пустыми строками между):
Последствия:
Иван: Инвентарь — Потерян предмет: Нож охотничий

Мария: Здоровье — Грипп (легкая) → Грипп (средняя)

Группа: Запас еды — -3 месяца
`;

  // Под конец — даём список раскрытых характеристик (только их можно менять явно)
  if (Object.keys(revealedChars).length > 0) {
    prompt += `\n=== РАСКРЫТЫЕ ХАРАКТЕРИСТИКИ АКТИВНЫХ ИГРОКОВ (ТОЛЬКО ИХ МОЖНО МЕНЯТЬ ЯВНО) ===\n`;
    Object.entries(revealedChars).forEach(([playerName, chars]) => {
      const charStrings = Object.entries(chars).map(([k, v]) => `${k}: ${v}`);
      prompt += `- ${playerName}: ${charStrings.join(', ')}\n`;
    });
  } else {
    prompt += `\n=== РАСКРЫТЫХ ХАРАКТЕРИСТИК НЕТ ===\nНельзя менять характеристики игроков. Можно менять только общие последствия (еда/бункер), если это логично.\n`;
  }
