...
const ABILITY_LIST = [
  ...
  "Сеанс психотерапии: раскрыть фобию другого игрока и один раз за игру вылечить её (нельзя примнеить к себе)",
  "Общий наркоз: выбранный игрок пропускает ход и голосование",
  "Сыворотка: добавляет себе в инвентарь вакцину от вируса",
  "Укрепление иммунитета: повышает сопротивляемость игрока ко всем имунно-инфекционным болезням и излечивает себя от них",
  "Орлиный глаз: позволяет игроку найти скрытые ресурсы или улики в локации (+6 мес. еды)",
  "Нашествие крыс: уменьшает запас еды в бункере на 6 месяцев",
  "Дезинфектор: обеззараживает предметы или ресурсы",
  "Любовный союз: связывает двух игроков — они не могут голосовать друг против друга 2 раунда",
  "Дефибриллятор: один раз за игру может вернуть к жизни игрока, умершего в этом раунде",
  "Ремонтный набор: чинит сломанное оборудование в бункере или предмет в инвентаре",
  "Восстановление сети: возвращает электроснабжение, открывая доступ к электронным системам",
  "Силовое давление: заставляет игрока раскрыть одну скрытую характеристику или применить способность на нужную цель",
  "С чистого листа: сбрасывает все характеристики игроков и заменяет на новые",
  "Запугивание: запрещает выбранному игроку использовать способность против определённого игрока",
  "Фальсификация: меняет результат голосования на противоположный (можно применить после голосования)",
  "Прогноз погоды: предупреждает об опасных явлениях и выбирает безопасное окно для вылазок",
  "Картограф: знает месторасположение полезных ресурсов поблизости (водоемы, леса, пастбища, соседние бункеры)",
  "Укрепление убежища: повышает защиту бункера от внешних угроз",
  "Сабботаж: ломает важную систему бункера (вентиляция, электрика, генераторы, канализацию и т.д.)",
  "Ловкие пальчики: крадёт инвентарь у любого игрока",
  "Нападение зверя: на выбранного игрока нападает дикий зверь, игрок погибает с шансом 50%",
  "Барахольщик: рандомно изменяет инвентарь",
  "Наставничество: обучает игрока, временно повышая его стаж на 5 лет",
  "Переквалификация: случайным образом меняет свою профессию или другого игрока, но максимальный стаж - 1 год",
  "Переработка: вы научились перерабатывать отходы в энергию, которой хватит до конца времянахождения в бункере",
  "Глаз бога: раскрывает пол игрока",
  "Сплетничество: раскрывает доп. сведения любого игрока",
  "Неуязвимость: против вас нельзя использовать негативные способности и выгнать из бункера на 1 раунд",
  "Говорит с акцентом: до конца игры выбранный игрок общается с иностранным акцентом (по выбору обладателя способности)",
  "Заикание: в начале каждого предложения выбранный игрок заикается на первой букве в течение трёх секунд",
  "Шёпот: выбранный игрок может говорить только шёпотом в течение 2 раундов, настолько тихо, что другие игроки должны напрягать слух, чтобы его услышать",
  "Рифмоплёт: выбранный игрок обязан говорить только в рифму в течение 1 раунда, иначе молчать 2 раунда",
  "Туберкулёз: выбранный  игрок постоянно кашляет на протяжении 2х раундов, перебивая свою же речь, и не может закончить предложение без приступа",
  "Заразительный смех: когда выбранный игрок начинает говорить, он не может сдержать смех — в середине предложения он должен заливаться смехом минимум на 2 секунды",
  "Пикми: когда выбранный игрок отстаивает свою позицию, он должен говорить в стиле Пикми - до конца игры",
  "Зажатый нос: игрок зажимает нос пальцами и говорит гнусаво 2 раунда",
  "Заражение: добавляет в общий запас еды испорченный продукт — один случайный игрок отравится, если поест и погибнет с вероятностью 50%",
