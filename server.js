...
6. Последствия должны быть КОНКРЕТНЫМИ и добавляться явно:
   - Здоровье: конкретное изменение болезни (например "Астма (легкая) → Астма (средняя)")
   - Инвентарь: "Добавлен предмет: [название]" или "Потерян предмет: [название]" (только если был в инвентаре)
   - Еда: +X месяцев / -X месяцев (макс +2 года, -1 год)
   - Бункер: конкретное изменение характеристики
   - Фобия: "Добавлена фобия: [название]"

7. Здоровье может меняться только в рамках:
   - Ухудшение/улучшение текущей степени болезни
   - Полное излечение (на "Здоров")
   - Добавление новой болезни со степенью (легкая/средняя/тяжелая/критическая)
...
`;
  ...
  prompt += `\n=== ФОРМАТ ОТВЕТА ===
4-5 предложений с описанием события.

<br> Последствия:
<br> - Имя игрока: конкретное изменение здоровья
<br> - Имя игрока: добавлена фобия: [название] (если есть)
<br> - Инвентарь: добавлен предмет: [название] / потерян предмет: [название] (только если был)
<br> - Запас еды: +X месяцев / -X месяцев
<br> - Бункер: изменение (если есть)
<br> - Другое: ... (если есть)

🔴 ВАЖНО: Сгенерируй событие с ОБЯЗАТЕЛЬНЫМ пошлым/сексуальным элементом и позорным моментом для игрока!
🔴 НЕ ЗАСТАВЛЯЙ игроков терять то, чего у них нет в инвентаре!`;
